...
export default function HistoryDetail({ params }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Make the API request to get soil test record details
      const response = await fetch(`${API_URL}/api/soil/${id}`, {
        method: 'GET',
        headers: {
      ...
      if (!data || !data.success) {
        throw new Error(data?.message || 'Failed to fetch soil record');
      }
      
      // Set soil record data
      setSoilRecord(data.data);
      
      // Fetch recommendations for the soil test
      const recResponse = await fetch(`${API_URL}/api/recommendations/${id}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      // If API call fails in development, use mock data
      if (process.env.NODE_ENV === 'development') {
        console.log('Using mock data in development environment');
        
        // Mock soil record data
          ...
          fertilizers: [
            {
              ...
              total_amount: 37.5 * (mockSoilRecord.land_area || 1)
            }
          ],
          management_tips: [
            {
        ...
        setError(err.message || trans.loadingError);
      }
    } finally {
      setLoading(false);
    }
  ...
  // Prepare chart data for nutrient levels
  ...
  const handleDelete = useCallback(async () => {
    try {
      ...
      // Make API request to delete soil record
      const response = await fetch(`${API_URL}/api/soil/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
      ...
      // Show success message using alert (could be replaced with a toast notification)
      ...
      // In development, just navigate back for testing
      if (process.env.NODE_ENV === 'development') {
        alert(trans.deleteModal.success);
        router.push('/history');
      } else {
  ...
  // No record found state renderer
  if (!soilRecord) {
    return (
      <ThemeWrapper>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
