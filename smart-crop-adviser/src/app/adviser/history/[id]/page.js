...
export async function generateStaticParams() {
  // Try to fetch available prediction IDs for static export.
  // If the API is inaccessible during build, return an empty array (no static pages).
  try {
    const res = await fetch(`${API_BASE}/api/v1/predictions`, { cache: 'no-store' });
    if (!res.ok) {
      console.warn('generateStaticParams: failed to fetch list, status', res.status);
      return [];
    }
    const json = await res.json();
    const list = json?.data ?? json ?? [];
    // Map to params. Make sure ids are strings.
    return list.map((item) => ({ id: String(item.id ?? item._id) }));
  } catch (e) {
    console.warn('generateStaticParams error:', e);
    return [];
  }
}
...
export default async function Page({ params }) {
  ...
  // Try to fetch initial data server-side to pass as prop (optional).
  // This helps render content immediately for static export (if available).
  let initialData = null;
  try {
    const res = await fetch(`${API_BASE}/api/v1/predictions/${id}`, { cache: 'no-store' });
    if (res.ok) {
      const json = await res.json();
      initialData = json?.data ?? json ?? null;
    }
  } catch (e) {
    // ignore, client will fetch
  }

  // Render client component and pass id + initialData
  return <HistoryDetailClient id={String(id)} initialData={initialData} />;
}
