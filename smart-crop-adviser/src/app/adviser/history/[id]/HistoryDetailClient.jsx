// app/adviser/history/[id]/HistoryDetailClient.jsx
'use client';
...
export default function HistoryDetailClient({ id, initialData = null }) {
  ...
  // Fetch detail (client-side). If server-side initialData provided, use it first.
  const fetchDetail = useCallback(async () => {
    ...
    try {
      ...
      const normalized = {
        ...
        phosphorus: safeNumber(r.phosphate ?? r.phosphorus),
      ...
      setRecs({
        crops: normalized.crop_name ? [{ name: normalized.crop_name, suitability: 'Good', reason: 'Based on recorded crop name' }] : [],
        soil_issues: (r.soil_issues && Array.isArray(r.soil_issues)) ? r.soil_issues : [],
        fertilizers: (r.fertilizers && Array.isArray(r.fertilizers)) ? r.fertilizers : [],
        management_tips: (r.management_tips && Array.isArray(r.management_tips)) ? r.management_tips : []
      });
    } catch (e) {
      console.error(e);
  ...
  useEffect(() => {
    // If initialData exists, use it immediately while still fetching latest in background.
    if (initialData) {
      try {
        const r = initialData;
        const normalized = {
    ...
    // fetch newest data client-side to keep up-to-date
    fetchDetail();
  ...
  const translateSuitability = (s) => {
    const map = { Excellent: { si: 'විශිෂ්ට', ta: 'சிறந்தது' }, Good: { si: 'හොඳයි', ta: 'நல்லது' }, Fair: { si: 'සාධාරණයි', ta: 'நியாயமானது' } };
    if (language === 'en') return s; return map[s]?.[language] || s;
  };
  const translateSeverity = (sev) => {
    const map = { High: { si: 'ඉහළ', ta: 'உயர்' }, Medium: { si: 'මධ්‍යම', ta: 'நடுத்தர' }, Low: { si: 'අඩු', ta: 'குறைந்த' } };
    if (language === 'en') return sev; return map[sev]?.[language] || sev;
  };
  const translateCropType = (ct) => {
    const map = { General: { si: 'සාමාන්‍ය', ta: 'பொது' } };
    if (language === 'en') return ct; return map[ct]?.[language] || ct;
  };
    ...
    return (
      <ThemeWrapper>
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="rounded-md p-4" style={{ backgroundColor: isDark ? 'rgba(220,38,38,0.2)' : '#FEF2F2', borderLeftWidth: 4 }}>
            <div className="flex items-center gap-3">
              <AlertTriangle />
              <p style={{ color: isDark ? '#F87171' : '#B91C1C' }}>{err}</p>
            </div>
          </div>
          <div className="mt-4">
            <Link href="/adviser/history" className="inline-flex items-center px-4 py-2 rounded-md text-white" style={{ backgroundColor: theme.colors.primary }}>
              <ArrowLeft className="mr-2" /> {trans.backToHistory}
            </Link>
          </div>
        </div>
      </ThemeWrapper>
  ...
  return (
    <ThemeWrapper>
      <div className="max-w-6xl mx-auto px-4 py-6">
        {/* ... the same UI as before: header, chart, lists, params, modal ... */}
        {/* For brevity I keep the same markup structure as your previous component,
            but here is the essential: header, recommended crops, chart, soil issues,
            fertilizers, management tips, soil parameters, footer actions and delete modal. */}
        ...
        <div className="rounded-lg shadow-lg p-6 mb-6" style={{ backgroundColor: theme.colors.card, borderColor: theme.colors.border }}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-3xl font-bold" style={{ ...getTextStyle({ color: theme.colors.text }), ...contentStyle }}>{trans.title}</h1>
            </div>
            <div className="flex space-x-2">
              <Link href="/adviser/history" className="p-2 rounded-full" style={{ backgroundColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)', color: theme.colors.text }} title={trans.backToHistory}>
                <ArrowLeft />
              </Link>
              <button onClick={handlePrint} className="p-2 rounded-full" title={trans.printRecord}><Printer /></button>
              <button onClick={() => setShowDelete(true)} className="p-2 rounded-full" title={trans.deleteRecord}><Trash2 style={{ color: '#EF4444' }} /></button>
            </div>
          </div>
          <div className="flex items-center mt-2">
            <Calendar className="mr-2" />
            <p>{new Date(record.created_at).toLocaleDateString()} — {record.district}{record.agro_zone ? `, ${record.agro_zone}` : ''}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-6">
            {/* Crops */}
            <div className="rounded-lg shadow-lg p-6 border" style={{ backgroundColor: theme.colors.card }}>
              <h2 className="text-xl font-semibold mb-4 flex items-center gap-2"><Leaf /> {trans.sections.recommendedCrops}</h2>
              <div className="space-y-3">
                {(recs?.crops ?? []).map((crop, i) => {
                  const color = getSuitabilityColor(crop.suitability);
                  return (
                    <div key={i} className="border rounded-lg p-4" style={{ borderColor: theme.colors.border }}>
                      <div className="flex justify-between items-start mb-2">
                        <h3 className="font-medium">{crop.name}</h3>
                        {crop.suitability && <span className="px-2 py-1 rounded-full text-xs font-medium" style={{ backgroundColor: color.bg, color: color.text }}>{translateSuitability(crop.suitability)}</span>}
                      </div>
                      {crop.reason && <p className="text-sm">{crop.reason}</p>}
                    </div>
                  );
                })}
                {(!recs || (recs.crops || []).length === 0) && <p className="text-sm">—</p>}
              </div>
            </div>

            {/* Chart */}
            <div className="rounded-lg shadow-lg p-6 border" style={{ backgroundColor: theme.colors.card }}>
              <h2 className="text-xl font-semibold mb-4 flex items-center gap-2"><BarChart3 /> {trans.sections.nutrientLevels}</h2>
              <div className="h-64"><Line data={nutrientChartData} options={chartOptions} /></div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            {(recs?.soil_issues || []).length > 0 && (
              <div className="rounded-lg shadow-lg p-6 border" style={{ backgroundColor: theme.colors.card }}>
                <h2 className="text-xl font-semibold mb-4 flex items-center gap-2"><AlertTriangle /> {trans.sections.soilConsiderations}</h2>
                  ...
                  {(recs?.soil_issues || []).map((issue, i) => (
                    <div key={i} className="flex items-start gap-2 p-3 rounded-lg">
                      ...
                      <div>
                        ...
                        {issue.severity && <span className="ml-2 px-1.5 py-0.5 text-xs rounded">{translateSeverity(issue.severity)}</span>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {(recs?.fertilizers || []).length > 0 && (
              <div className="rounded-lg shadow-lg p-6 border" style={{ backgroundColor: theme.colors.card }}>
                <h2 className="text-xl font-semibold mb-4 flex items-center gap-2"><Droplets /> {trans.sections.fertilizerRecommendations}</h2>
                <div className="space-y-4">
                  {(recs?.fertilizers || []).map((f, i) => (
                    <div key={i} className="flex items-start gap-2 p-3 rounded-lg"><CheckCircle /><span>{f.fertilizer}</span></div>
                  ))}
                </div>
              </div>
            )}
            ...
            {(recs?.management_tips || []).length > 0 && (
              <div className="rounded-lg shadow-lg p-6 border" style={{ backgroundColor: theme.colors.card }}>
                <h2 className="text-xl font-semibold mb-4 flex items-center gap-2"><Thermometer /> {trans.sections.managementTips}</h2>
                <div className="space-y-2">
                  {(recs?.management_tips || []).map((tip, i) => (
                    <div key={i} className="flex items-start gap-2 p-3 rounded-lg"><CheckCircle /><div><span>{tip.tip}</span></div></div>
                  ))}
                </div>
              </div>
            )}
            ...
            {/* Soil parameters */}
            <div className="rounded-lg shadow-lg p-6 border" style={{ backgroundColor: theme.colors.card }}>
              <h2 className="text-xl font-semibold mb-4 flex items-center gap-2"><MapPin /> {trans.sections.soilParameters}</h2>
        ...
        <div className="mt-8 flex justify-between">
          <Link href="/adviser/history" className="inline-flex items-center px-4 py-2 rounded-md text-white" style={{ backgroundColor: theme.colors.primary }}>
            <ArrowLeft className="mr-2" /> <span>{trans.backToHistory}</span>
          </Link>
          <button onClick={() => setShowDelete(true)} className="inline-flex items-center px-4 py-2 rounded-md text-white" style={{ backgroundColor: '#EF4444' }}>
            <Trash2 className="mr-2" /> <span>{trans.deleteRecord}</span>
          </button>
        </div>
        ...
        {showDelete && (
          <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-10">
            <div className="max-w-md w-full p-6 rounded-lg shadow-xl" style={{ backgroundColor: theme.colors.card }}>
              <h3 className="text-lg font-medium">{trans.deleteModal.title}</h3>
              <p className="mt-2">{trans.deleteModal.message}</p>
              <div className="mt-4 flex justify-end space-x-3">
                <button onClick={() => setShowDelete(false)} className="px-4 py-2">{trans.deleteModal.cancel}</button>
                <button onClick={handleDelete} className="px-4 py-2 text-white" style={{ backgroundColor: '#EF4444' }}>{trans.deleteModal.confirm}</button>
              </div>
            </div>
          </div>
        )}
...
function ParamRow({ label, value }) {
  ...
  const getTextStyle = (base = {}) => {
    const lh = language === 'si' ? 1.7 : language === 'ta' ? 1.8 : 1.5;
    return { ...base, lineHeight: lh };
  };
  return (
    <div>
      <span style={{ ...getTextStyle({ color: isDark ? 'rgba(255,255,255,0.6)' : 'rgba(0,0,0,0.6)' }) }}>{label}</span>
      <span className="ml-2 font-medium" style={{ color: theme.colors.text }}>{value}</span>
    </div>
  );
}
