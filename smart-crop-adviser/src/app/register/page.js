...
// API URL - you should set this in your environment variables
// Adjust this URL to match your actual backend server address
const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001';
// Enable this for local development with separate frontend/backend servers
const USE_PROXY = false; // Set to true if using Next.js API routes as a proxy
...
const translations = {
  ...
  si: {
    ...
    form: {
      ...
      userLevel: {
        label: "පරිශීලක වර්ගය",
        placeholder: "පරිශීලක වර්ගය තෝරන්න",
        options: {
          farmer: "ගොවියා",
          agent: "ව්‍යාප්ති නියෝජිත",
          researcher: "පර්යේෂක",
          admin: "පරිපාලක"
        },
        errors: {
          required: "පරිශීලක වර්ගය අවශ්‍යයි"
        }
      },
      terms: {
        label: "මම එකඟ වෙමි",
    ...
    errors: {
      ...
      usernameExists: "පරිශීලක නාමය දැනටමත් පවතී",
      emailExists: "විද්‍යුත් තැපෑල දැනටමත් පවතී",
      serverError: "සේවාදායක දෝෂයකි. පසුව නැවත උත්සාහ කරන්න."
    },
    success: {
      accountCreated: "ගිණුම සාර්ථකව නිර්මාණය කරන ලදී! පිවිසුමට යොමු වෙමින්..."
    }
  },
  ta: {
    ...
    form: {
      ...
      userLevel: {
        label: "பயனர் வகை",
        placeholder: "பயனர் வகையைத் தேர்ந்தெடுக்கவும்",
        options: {
          farmer: "விவசாயி",
          agent: "விரிவாக்க முகவர்",
          researcher: "ஆராய்ச்சியாளர்",
          admin: "நிர்வாகி"
        },
        errors: {
          required: "பயனர் வகை தேவை"
        }
      },
      terms: {
    ...
    errors: {
      ...
      usernameExists: "பயனர்பெயர் ஏற்கனவே உள்ளது",
      emailExists: "மின்னஞ்சல் ஏற்கனவே உள்ளது",
      serverError: "சேவையக பிழை. பின்னர் மீண்டும் முயற்சிக்கவும்."
    },
    success: {
      accountCreated: "கணக்கு வெற்றிகரமாக உருவாக்கப்பட்டது! உள்நுழைவுக்கு வழிநடத்துகிறது..."
    }
  }
};
...
export default function Register() {
  ...
  const [formData, setFormData] = useState({
    ...
    location: '',
    farmSize: '',
    userLevel: 'farmer', // Default user level
    termsAccepted: false
  });
  ...
  const validateForm = () => {
    ...
    if (!formData.userLevel) {
      errors.userLevel = trans.form.userLevel.errors.required;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Prepare registration data mapping frontend field names to backend field names
      const registrationData = {
        username: formData.username,
        email: formData.email,
        password: formData.password,
        address: formData.location,          // Map location to address
        landsize: formData.farmSize ? parseFloat(formData.farmSize) : null, // Map farmSize to landsize
        userlevel: formData.userLevel        // Required field for backend
      };
      
      console.log('Sending registration data:', registrationData);
      
      // Determine the API endpoint based on configuration
      let apiEndpoint = `${API_URL}/api/v1/users`;
      
      // For local development with Next.js API routes as proxy
      if (USE_PROXY) {
        apiEndpoint = '/api/proxy/users'; // This would be handled by a Next.js API route
      }
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        throw new Error(`Server returned non-JSON response. Status: ${response.status}. Check if the API server is running at ${API_URL}.`);
      }
      ...
      if (!response.ok) {
        ...
        if (response.status === 409) {
          ...
          if (data.errors && data.errors.length > 0) {
            const usernameError = data.errors.find(e => e.field === 'username');
            const emailError = data.errors.find(e => e.field === 'email');
            
            if (usernameError) {
              setError(trans.errors.usernameExists);
            } else if (emailError) {
              setError(trans.errors.emailExists);
            } else {
              setError(data.message || trans.errors.general);
            }
          } else {
            setError(data.message || trans.errors.general);
          }
        } else if (response.status === 404) {
          setError(`API endpoint not found at ${apiEndpoint}. Please check the server configuration.`);
        } else {
          setError(data.message || trans.errors.serverError);
        }
        return;
      }
      
      // Registration successful
      setSuccess(trans.success.accountCreated);
      ...
      setError(error.message || trans.errors.serverError);
    } finally {
      setLoading(false);
    }
  };
              ...
              <div className={`mb-4 border-l-4 p-4 rounded ${
                ...
                <div className="flex">
                  <div className="flex-shrink-0">
                    <CheckCircle className={`h-5 w-5 ${isDark ? 'text-green-400' : 'text-green-400'}`} />
                  </div>
                  <div className="ml-3">
                    <p 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label 
                    ...
                    style={{ ...contentStyle, ...getTextStyle({ color: theme.colors.text }) }}
                  >
                    {trans.form.userLevel.label} <span className="text-red-500">*</span>
                  </label>
                  <div className="mt-1 relative rounded-md shadow-sm">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                      <BadgeCheck className={`h-5 w-5 ${isDark ? 'text-gray-500' : 'text-gray-400'}`} />
                    </div>
                    <select
                      id="userLevel"
                      name="userLevel"
                      ...
                      onChange={handleChange}
                      className={`appearance-none block w-full pl-10 pr-3 py-2 rounded-md focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm ${
                        validationErrors.userLevel ? 'border-red-300' : ''
                      }`}
                      style={{ 
                        backgroundColor: isDark ? 'rgba(255,255,255,0.05)' : 'white',
                        color: theme.colors.text,
                        borderColor: validationErrors.userLevel
                          ? 'rgb(252, 165, 165)'
                          : isDark ? 'rgba(255,255,255,0.1)' : theme.colors.border
                      }}
                    >
